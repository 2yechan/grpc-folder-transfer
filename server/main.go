...
func (s *FolderServer) UploadFolder(stream pb.FolderTransfer_UploadFolderServer) error {
  ...
  defer func() {
    for _, f := range fileHandles { f.Close() }
  }()
  ...
  for {
    ...
    if chunk.IsLast {
      ...
      log.Printf("[Upload] 파일 완료: %s", chunk.FilePath)
    }
  }
...
func main() {
  ...
  if err != nil { log.Fatalf("Listen 실패: %v", err) }
